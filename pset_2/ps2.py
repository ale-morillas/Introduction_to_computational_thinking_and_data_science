...
# represented?
#
# Answer: The graph's nodes represents the MIT buildings, the edges are the  
# conections between those biuldings, from source to destination in distance,
# and the distances are the total distance between biuldings and the distance outdoors.
...
def test_road_map(file):
    test = open(file, 'w+')         # create a new txt file
    map = load_map('mit_map.txt')
    for node in map.__str__(): 
        test.write(node)            # for each node in map write it in the txt file
    test.close()
test_road_map('test_road_map.txt')
