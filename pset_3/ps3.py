...
class RectangularRoom(object):
    ...
    def __init__(self, width, height, dirt_amount):
        ...
        """
        if not type(width) == int or width <= 0: # making sure that the argument was passed as integer.
            raise ValueError
        else:
            self.width = width      
        if not type(height) == int or height <= 0: # making sure that the argument was passed as integer.
            raise ValueError
        else:
            self.height = height
        if not type(dirt_amount) == int or dirt_amount < 0: # making sure that the argument was passed as integer.
            raise ValueError
        else:
            self.dirt_amount = dirt_amount
            
        self.tiles = {}
        for x in range(width):  # creating grid of tiles.
    ...
    def clean_tile_at_position(self, pos, capacity):
        ...
        """
        if pos.get_x() > self.width: # checking if position x value is not out of room range.
            raise ValueError
        if pos.get_y() > self.height: # checking if position x value is not out of room range.
            raise ValueError
        # tile is described as tuple of integers and NOT floats, we do floor value of float to get it in range of tile.
        pos_tile = (math.floor(pos.get_x()), math.floor(pos.get_y())) 
        # if cleaning capacity is bigger than the amt of dirt the tile is considered as cleaned(which is 0 dirt).
        if self.tiles[pos_tile] < capacity:
            self.tiles[pos_tile] = 0
        else:
             self.tiles[pos_tile] -= capacity
...
class Robot(object):
    ...
    def __init__(self, room, speed, capacity):
        ...
        """
        self.room = room
        if speed <= 0:  # speed can not be negative!
            raise ValueError
        else:
            self.speed = speed
        if not type(capacity) == int or capacity < 0: # capacity can not be negative, robot is not allowed to add dirt to the tile!
            raise ValueError
        else:
...
class FurnishedRoom(RectangularRoom):
    ...
    def is_position_furnished(self, pos):
        ...
        """
        pos_tile = (math.floor(pos.get_x()), math.floor(pos.get_y()))
        return pos_tile in self.furniture_tiles
    ...
    def get_num_tiles(self):
        ...
        """
        total_tiles = len(self.tiles.keys()) - len(self.furniture_tiles)
        return total_tiles
    ...
    def get_random_position(self):
        ...
        """
        while True:
            random_pos = Position(random.uniform(0, self.width), random.uniform(0, self.height))
...
class StandardRobot(Robot):
    ...
    def update_position_and_clean(self):
        ...
        if self.room.is_position_valid(new_pos):  # if robot's postion is valid robot can take action and clean,
            self.set_robot_position(new_pos)
            self.room.clean_tile_at_position(self.position, self.capacity)
        else:               # otherwise his direction will be changed randomly and he will not take any action (he will not clean).
...
def run_simulation(num_robots, speed, capacity, width, height, dirt_amount, min_coverage, num_trials,
    ...
    anim = ps3_visualize.RobotVisualization(num_robots, width, height, False, delay=0.5)
    avg_num_of_steps = []   # average number of steps for trial will be stored in this list.
    for index in range(num_trials):
        room = EmptyRoom(width, height, dirt_amount)  # by default we will simulate EmptyRoom type.
        amt_of_tiles = room.get_num_tiles()  # storing amount of tiles in the room.
        cleaned_tiles = room.get_num_cleaned_tiles()  # storing amount of cleaned tiles in the room.
        ...
        while True:
            ...
            if cleaned_tiles / amt_of_tiles >= min_coverage:
                ...
                for robot in my_robots:
                    robot.update_position_and_clean()  # taking cleaning action (step).
                    cleaned_tiles = room.get_num_cleaned_tiles() # updating number of cleaned tiles in the room.
                # updating animation, please comment out line below if animation was disabled.
                anim.update(room, my_robots)
                num_of_steps += 1
        avg_num_of_steps.append(num_of_steps)
    # closing animation, please comment this line out if animation was disabled at the beginning.
    anim.done()
    return sum(avg_num_of_steps) / len(avg_num_of_steps)
